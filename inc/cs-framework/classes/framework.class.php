...
class CSFramework extends CSFramework_Abstract {
  ...
  public function validate_save( $request ) {
    ...
    if ( isset( $request['import'] ) && ! empty( $request['import'] ) ) {
      ...
      $add_errors[] = $this->add_settings_error( __( 'Success. Imported backup options.', 'doyel' ), 'updated' );
    }

    // reset all options
    if ( isset( $request['resetall'] ) ) {
      $add_errors[] = $this->add_settings_error( __( 'Default options restored.', 'doyel' ), 'updated' );
      return;
    }
    ...
    if ( isset( $request['reset'] ) && ! empty( $section_id ) ) {
      ...
      $add_errors[] = $this->add_settings_error( __( 'Default options restored for only this section.', 'doyel' ), 'updated' );
    }
  ...
  public function admin_page() {
    ...
    echo '<div class="cs-framework cs-option-framework">';
      ...
      echo ( $this->settings['ajax_save'] ) ? '<span id="cs-save-ajax">'. __( 'Settings saved.', 'doyel' ) .'</span>' : '';

      submit_button( __( 'Save', 'doyel' ), 'primary cs-save', 'save', false, array( 'data-save' => __( 'Saving...', 'doyel' ) ) );
      submit_button( __( 'Restore', 'doyel' ), 'secondary cs-restore cs-reset-confirm', $this->unique .'[reset]', false );

      if( $this->settings['show_reset_all'] ) {
        submit_button( __( 'Reset All Options', 'doyel' ), 'secondary cs-restore cs-warning-primary cs-reset-confirm', $this->unique .'[resetall]', false );
      }

      echo '</fieldset>';
      echo ( empty( $has_nav ) ) ? '<a href="#" class="cs-expand-all"><i class="fa fa-eye-slash"></i> '. __( 'show all options', 'doyel' ) .'</a>' : '';
